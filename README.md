...
- ✅ Команды Telegram-бота в polling-режиме (`/start`, `/help`, `/today`, `/chart`, `/last`, `/web`, `/subscribe_here`, `/unsubscribe_here`, `/status`)
- ✅ Команда ежедневной публикации в Telegram (`vrw post-daily`)
- ✅ Документация по деплою в Railway и настройке расписаний
- ✅ Веб-дашборд поддерживает светлую (по умолчанию) и тёмную тему с переключателем
...
Если задана `WEB_APP_URL`, бот будет показывать кнопку «Открыть веб-график» и поддерживать команду `/web`.

Подписки для групповых чатов:
- В личных сообщениях чат по-прежнему автоматически считается активной подпиской.
- В группах/супергруппах подписка включается вручную командой `/subscribe_here` в нужном чате.
- Команда `/unsubscribe_here` отключает подписку текущего чата.
- Команда `/status` показывает, подписан ли текущий чат на ежедневный график.
- Если во время `vrw post-daily` бот больше не может писать в чат, этот чат автоматически деактивируется, а в результате команды появляется понятное сообщение с chat_id проблемного чата.
...
- Отправляет текстовое уведомление в Telegram только активным личным чатам (`chat_type=private`) с компактной сводкой изменений (`changed/new/removed`).
...
- `WEB_APP_URL=<публичный URL web-сервиса, например https://vrw-web.up.railway.app/>`
- `SCRAPE_TIMES_UTC=every hour (0 * * * *)`
- `DAILY_POST_TIME_UTC=19:00`

### 3.1) Как включить `/web` и кнопку «Открыть веб-график» в Telegram-боте (Railway, пошагово)

Ниже — минимальный сценарий, после которого функция точно заработает.

1. Убедитесь, что web-сервис (`vrw-web`) уже публичный:
   - `Networking` → `Public Networking` → `Generate Domain`.
   - Проверьте, что открывается `https://<ваш-web-домен>/health` и возвращает `{"status":"ok"}`.

2. Скопируйте базовый URL web-сервиса (без дополнительных путей), например:
   - `https://vrw-web-production-xxxx.up.railway.app`

3. Откройте сервис бота (`vrw-bot`) в Railway → `Variables` и добавьте:

   ```env
   WEB_APP_URL=https://vrw-web-production-xxxx.up.railway.app
   ```

   > Важно: задавайте именно в **bot service**, а не только в `vrw-web`.

4. Перезапустите `vrw-bot` (Deploy/Restart), чтобы переменная подтянулась в процесс.

5. Проверьте в Telegram:
   - отправьте `/start` или `/help` — должна появиться inline-кнопка **«Открыть веб-график»**;
   - отправьте `/web` — бот должен прислать ссылку на web-дашборд.

6. Если кнопка/команда не появились:
   - проверьте, что `WEB_APP_URL` не пустой и начинается с `https://`;
   - проверьте, что редеплой именно `vrw-bot` завершился успешно;
   - проверьте, что URL из `WEB_APP_URL` реально открывается в браузере.
